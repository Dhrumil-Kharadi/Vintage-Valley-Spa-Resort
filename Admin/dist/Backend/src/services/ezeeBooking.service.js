...
exports.createAndConfirmBooking = createAndConfirmBooking;
...
function assertNonEmpty(val, name) {
    if (!val || val.trim() === "") {
        throw new errorHandler_1.HttpError(400, `${name} cannot be empty`);
    }
    return val.trim();
}
/** Truncate JSON for safe log embedding */
function safeJsonSnippet(data) {
    try {
...
function unwrapEzeePayload(payload) {
    return Array.isArray(payload) ? payload[0] : payload;
}
function extractReservationNo(payload) {
    const p = unwrapEzeePayload(payload);
    if (!p)
        return null;
    if (typeof p.ReservationNo === "string")
        return p.ReservationNo;
    if (Array.isArray(p.ReservationNo) && p.ReservationNo.length > 0) {
        return String(p.ReservationNo[0]);
    }
    return null;
}
function mapEzeeErrorDetailsToHttpError(payload, context) {
    const p = unwrapEzeePayload(payload);
    ...
    if (code === "AuthenticationFailed")
        statusCode = 401;
    else if (code === "AuthorizationFailed")
        statusCode = 403;
    else if (code === "RecordNotFound")
        statusCode = 404;
    else if (code === "RatePlanNotAvailable" || code === "RoomNotAvailable")
        statusCode = 409;
    else if (code === "InvalidRequest" || code === "ParametersMissing")
        statusCode = 400;
    else if (code === "InternalError")
        statusCode = 500;
    else
        statusCode = 400;
    return new errorHandler_1.HttpError(statusCode, `${code} ${message}`);
}
function mapEzeeErrorToHttpError(payload, context) {
    ...
    return new errorHandler_1.HttpError(502, `${context}: ${message}`);
}
function pad2(n) {
...
function toStrictDateStr(dateStr) {
    ...
    if (!Number.isFinite(d.getTime())) {
        throw new errorHandler_1.HttpError(400, `Invalid date format: "${s}" — expected YYYY-MM-DD`);
    }
    return `${d.getFullYear()}-${pad2(d.getMonth() + 1)}-${pad2(d.getDate())}`;
}
// ─── Rate helpers ────────────────────────────────────────────────────
function buildRateCsv(perNightRates, checkIn, nights, fallback) {
    const values = [];
...
function extractPerNightRatesFromRoom(ezeeRoom, _checkIn, _nights) {
    const rates = {};
    if (!ezeeRoom?.room_rates_info)
        return rates;
...
function extractPerNightRatesFromExtra(extraRatesInfo, checkIn, nights) {
    const rates = {};
    if (!extraRatesInfo)
        return rates;
...
function pickSingleRate(perNightRates) {
...
// ─── URL builder ─────────────────────────────────────────────────────
function buildEzeeReservationApiUrl(requestType, query) {
    const base = new URL("booking/reservation_api/listing.php", env_1.env.EZEE_BASE_URL);
    base.searchParams.set("request_type", requestType);
    base.searchParams.set("HotelCode", env_1.env.EZEE_HOTEL_CODE || "");
    base.searchParams.set("APIKey", env_1.env.EZEE_API_KEY || "");
...
function sanitizePayload(obj) {
    return JSON.parse(JSON.stringify(obj, (_key, value) => {
        if (value === undefined)
            return undefined; // JSON.stringify already drops undefined
        if (value === null)
            return ""; // null → empty string for eZee
        return value;
    }));
}
/** Diagnose what might be wrong with a booking payload — returns list of issues */
function diagnosePayload(data, nights) {
    const issues = [];
    // Top-level checks
    if (!data.check_in_date)
        issues.push("check_in_date is missing or empty");
    if (!data.check_out_date)
        issues.push("check_out_date is missing or empty");
    if (!DATE_REGEX.test(data.check_in_date))
        issues.push(`check_in_date "${data.check_in_date}" is not YYYY-MM-DD`);
    if (!DATE_REGEX.test(data.check_out_date))
        issues.push(`check_out_date "${data.check_out_date}" is not YYYY-MM-DD`);
    if (!data.Email_Address)
        issues.push("Email_Address is missing or empty");
    ...
    if (!data.Room_Details) {
        ...
        const rooms = Array.isArray(data.Room_Details) ? data.Room_Details : Object.values(data.Room_Details);
        if (rooms.length === 0)
            issues.push("Room_Details is empty (no rooms)");
        rooms.forEach((rd, idx) => {
            const label = `Room[${idx}]`;
            if (!rd) {
                issues.push(`${label} is null/undefined`);
                return;
            }
            if (!rd.Rateplan_Id)
                issues.push(`${label}.Rateplan_Id is missing/empty`);
            if (!rd.Ratetype_Id)
                issues.push(`${label}.Ratetype_Id is missing/empty`);
            if (!rd.Roomtype_Id)
                issues.push(`${label}.Roomtype_Id is missing/empty`);
            if (!rd.baserate)
                issues.push(`${label}.baserate is missing/empty`);
            if (rd.number_adults === undefined || rd.number_adults === "")
                issues.push(`${label}.number_adults is missing/empty`);
            if (rd.number_children === undefined)
                issues.push(`${label}.number_children is missing`);
            if (!rd.First_Name)
                issues.push(`${label}.First_Name is missing/empty`);
            if (!rd.Last_Name)
                issues.push(`${label}.Last_Name is missing/empty`);
    ...
    for (const [k, v] of Object.entries(data)) {
        if (v === undefined)
            issues.push(`Top-level field "${k}" is undefined`);
    }
    return issues;
}
// ─── Main booking function ───────────────────────────────────────────
async function createAndConfirmBooking(params) {
    console.log("🚀 [EZEE] ===== STARTING EZEE BOOKING PROCESS =====");
    // ── 1. Validate & normalise dates (STRICT YYYY-MM-DD) ──────────
    if (!params.checkIn || !params.checkOut) {
        throw new errorHandler_1.HttpError(400, "checkIn and checkOut dates are required");
    }
    ...
    if (checkIn < now)
        throw new errorHandler_1.HttpError(400, "Check-in date cannot be in the past");
    if (checkOut <= checkIn)
        throw new errorHandler_1.HttpError(400, "Check-out date must be after check-in date");
    ...
    if (!Number.isFinite(bookingPaymentMode) || bookingPaymentMode < 0) {
        throw new errorHandler_1.HttpError(400, "Invalid Booking_Payment_Mode");
    }
    if (rooms < 1)
    ...
    if (!params.ezeeRoom?.roomtypeunkid || !params.ezeeRoom?.roomrateunkid || !params.ezeeRoom?.ratetypeunkid) {
        throw new errorHandler_1.HttpError(400, "Missing eZee room mapping (roomtypeunkid/roomrateunkid/ratetypeunkid). These must come from the availability API response.");
    }
    const email = String(params.email ?? "").trim();
    if (!email)
        throw new errorHandler_1.HttpError(400, "Email_Address is required");
    ...
    if (allIdentical) {
        console.warn("⚠️ [EZEE] All 3 IDs are IDENTICAL (" + roomtypeId + "). " +
    ...
    if (issues.length > 0) {
        ...
        throw new errorHandler_1.HttpError(400, `eZee payload validation failed: ${issues.join("; ")}`);
    }
    // Room_Details must be object format with Room_1, Room_2 keys (per eZee docs)
    if (Array.isArray(cleanedBookingData.Room_Details)) {
        const obj = {};
        cleanedBookingData.Room_Details.forEach((r, i) => {
            obj[`Room_${i + 1}`] = r;
        });
    ...
    const insertUrlBase = new URL("booking/reservation_api/listing.php", env_1.env.EZEE_BASE_URL);
    const insertFormData = new URLSearchParams();
    insertFormData.append("request_type", "InsertBooking");
    insertFormData.append("HotelCode", env_1.env.EZEE_HOTEL_CODE || "");
    insertFormData.append("APIKey", env_1.env.EZEE_API_KEY || "");
    ...
    try {
        const response = await axios_1.default.post(insertUrlBase.toString(), insertFormData.toString(), {
            timeout: 30000,
        ...
        if (errDetails) {
            ...
            const errorCode = String(insertPayload?.["Error Details"]?.Error_Code ?? "").trim();
            ...
            throw new errorHandler_1.HttpError(errDetails.statusCode, `${errDetails.message}. bookingData=${safeJsonSnippet(cleanedBookingData)}`);
        }
        ...
        if (!reservationNo) {
            ...
            throw new errorHandler_1.HttpError(502, `eZee booking created but ReservationNo missing. payload=${safeJsonSnippet(insertPayload)}`);
        }
        ...
        processFormData.append("HotelCode", env_1.env.EZEE_HOTEL_CODE || "");
        processFormData.append("APIKey", env_1.env.EZEE_API_KEY || "");
        processFormData.append("Process_Data", JSON.stringify(processData));
        const processUrlBase = new URL("booking/reservation_api/listing.php", env_1.env.EZEE_BASE_URL);
        ...
        const processResponse = await axios_1.default.post(processUrlBase.toString(), processFormData.toString(), {
            timeout: 30000,
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
        });
        ...
        if (processErr) {
            ...
            throw new errorHandler_1.HttpError(processErr.statusCode, `${processErr.message}. reservationNo=${reservationNo}`);
        }
        if (processPayload?.Success === false || processPayload?.Error || processPayload?.ErrorCode) {
        ...
        throw new errorHandler_1.HttpError(502, `Failed to create booking in eZee: ${e.message}`);
    }
}
